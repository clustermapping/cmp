...
(function() {
    ...
    plot = d3Plot()
      ...
      .layer('avgGuideLayer',{
        ...
        elements:[
          {
            type: 'line',
            label: function() { return totalData.region + ' Average Wage: ' + numFormat(totalData.value);},
            axis: 'x',
            dataSource : function() {
        ...
        elements:[
          {
            ...
            append: [
              ...
              },
              {
                ...
                text: function(d){if (d) {return 'Wages by ' + d.type + ', ' + d.year;}}
              }]
          }
        ]})
      .layer('clusterDetail', {
  ...
  function getDatasources() {
    return base + '/report/cluster/wages' + window.location.hash.substring(1).split('?')[0];
  }
  ...
  function update() {
    ...
    loader.request(getDatasources(), function(){
      ...
      if (plotData.length < 15) {
        plot.height(81*plotData.length+plot.margin().top+plot.margin().bottom);
      } else {
        plot.height(1280);
      }
      plot.update();
