...
(function() {
  ...
  function buildChart(sel){
    ...
    plot = d3Plot()
      ...
      .scale("y", 'domain', function(data, key) { 
        ...
        if (data.length == 1) return [data[0][key] * 0.5, data[0][key] * 1.5]
        return domain;
      })
      .scale('y', 'format', '.2p')
      .scale('y', 'nice', 10)
  ...
  function getDatasources() {
    return base + '/report/region/specialization' 
      + '/' + options.regionType
      + '/' + options.regionId
      + '/' + options.start
      + '/' + options.end
