...
(function() {
  ...
  var MythosVis = window.MythosVis,
    ...
    plotData = [], totalData = {value: 0, benchmark: 0}, clusterData = [],
    yearRange, brush, gBrush,
    base = window.hbsBaseUrl || '',
    inverted = ['unemployment', 'poverty', 'unionization', 'taxes', 'corporate_taxes'],
    zoomed;

    defaults.regionTypes = {
       custom: { legend: 'economic areas', subtitle: 'Economic Area', count: 179 },
...
//        updateOverlay('clusterPopup', []);
    ...
    var marginTop = options.regionType == 'state' ? 135 : 155;
    plot = d3Plot()
      .height(700)
      .duration(250)
      ...
      .scale("y", 'range', function(data, key) {
        if (inverted.indexOf(options.indicator) >= 0) {
          return [0, (plot.drawHeight() - plot.margin.top() - plot.margin.bottom())];
        } else {
          return [(plot.drawHeight() - plot.margin.top() - plot.margin.bottom()), 0];
        }
      })
      ...
      .scale('x', 'nice', 4)
      .scale("x", 'range', function(data, key) {
        if (inverted.indexOf(options.indicator) >= 0) {
          return [(plot.drawWidth() - plot.margin.left() - plot.margin.right()), 0];
        } else {
          return [0, (plot.drawWidth() - plot.margin.left() - plot.margin.right())];
        }
      })
      .layer('axisLayer',{
        kind: 'axis',
        ...
        elements:[
          {
            ...
            label:function(d) {
              if (inverted.indexOf(options.indicator) >= 0) {
                return "Change in " + totalData.type.label + " Rate, " + totalData.start + ' to ' + totalData.end;
              } else {
                return "Growth in " + (totalData.type ? totalData.type.label : '') + ', ' + totalData.start + ' to ' + totalData.end;
              }
            },
            attrs:{
          ...
          {
            type:'y',
            label:function(d) { return (totalData.type ? totalData.type.label : '') + ' , ' + totalData.end; },
            axis:{
              fn: d3.svg.axis(),
              attrs:{orient:'left'}
            }
        ...
        elements:[
          {
            ...
            label: function() {
              if (inverted.indexOf(options.indicator) >= 0) {
                return totalData.benchmark + " Change in " + totalData.type.label + " Rate: " + percentPlustFormat(totalData.cagr);
              } else {
                return totalData.benchmark + " " + totalData.type.label + " Growth Rate: " + percentPlustFormat(totalData.cagr);
              }
            },
            attrs:{
              x1: 'x',
          ...
          },
          {
            ...
            label: function() {
              if (inverted.indexOf(options.indicator) >= 0) {
                ...
                return totalData.benchmark + " " + totalData.type.label + ": " + numFormat(totalData.value)
              }
            },
            attrs:{
              y1: 'y',
        ...
        elements:[
          {
            type: 'circle',
            attrs:{
              fill: function (d) { return (d.id == totalData.focus || totalData.focus.indexOf(d.id) > -1 ? '#f22': '#99f')},
              stroke: function (d) { return (d.id == totalData.focus || totalData.focus.indexOf(d.id) > -1 ? '#900': '#009')},
              r: 7,
              cx: 'x',
              cy: 'y',
              opacity: 1
        ...
        elements:[
          {
            ...
            append: [
              {
                type: 'h2',
                attrs: {class: 'title-text', style: 'border: 0; margin: 2px 0; padding-bottom: 0'},
                text: function(d) {
                  if (d) {
                    return d.regionName + " "
                      + (options.regionType == 'msa' || options.regionType == 'economic' ? (options.regionTypes[options.regionType].subtitle) : '' );
                  }
                }
              },
              {
                type: 'h2',
                attrs: {class: 'title-text', style: 'border: 0; margin: 2px 0; padding-bottom: 0'},
                text: function(d) { if (d) {return "Comparative " + (d.type? d.type.label : '') + " Performance"}}
              },
              {
                type: 'h2',
                attrs: {id: 'comparison-subtitle', class: 'title-text', style: 'border: 0; margin: 2px 0; padding-bottom: 0;font-size:18px;'},
                text: function(d) {return options.activeFilter || ""; }
              },
              {
                type: 'h3',
                ...
                append: [
                  ...
                  },
                  {
                    ...
                    attrs: {class: function (d) {
                      if (inverted.indexOf(options.indicator) >= 0) {
                        return 'indicator-value ' + (d.cagr < 0 ? 'positive' : 'negative');
                      } else {
                        return 'indicator-value ' + (d.cagr > 0 ? 'positive' : 'negative');
                      }
                    }},
                ...
                append: [
                  {
                    type: 'a',
                    attrs: {
                      href: function(d) { return (options.subcluster ? '/region/' + d.regionType + '/' + d.key : '/cluster/' + d.id); },
  ...
  function location(filter){
    var region = '/' + options.regionType + '/' + options.regionId,
      ...
      filter = '/' + options.filter + (options.subfilter ? '-' + options.subfilter : ''),
      hash = '#' + region +  years + indicator + filter + qops;
    return hash;
  }
  ...
  function updateOptionsFromHash(render) {
    var h = window.location.hash.substring(2),
      ...
      q = parseQuery(uq[1]);
    if (parts.length < 5) {
      console.error("Please specify a hash in the form: /:type/:code/:start/:end/:indicator");
    }
    ...
    options.end = parts[3];
    options.indicator = parts[4];
    options.filter = parts[5] ? parts[5].split('-')[0] : 'default';
    options.subfilter = parts[5] ? parts[5].split('-')[1] : undefined;
  ...
  function getDatasources() {
    return base + '/report/region/scorecard'
      + '/' + options.regionType
      + '/' + options.regionId
      + '/' + options.start
      + '/' + options.end
      + '/' + options.indicator
      + '/' + options.filter + (options.subfilter ? '-' + options.subfilter : '');
  }
  ...
  function filterControls() {
    if (options.regionType == 'state') return;
    var buttons = [],
      type = options.regionType;
    
    if (type == 'compare') {
      buttons.push({ name: 'States', code: 'state', items: [] });
      buttons.push({ name: 'Economic Areas', code: 'economic', items: [] });
      buttons.push({ name: 'MSAs', code: 'msa', items: [] });
      buttons.push({ name: 'Counties', code: 'county', items: [] });      
    }

    if (type == 'economic' || type == 'custom') {
      if (options.filter == 'default') {
        options.filter = 'type';
        options.subfilter = 'large';
      }

      buttons.push({name: 'Type', code: 'type', items: [
                    { name: 'Large Economic Areas', code: 'large', default: true },
                    { name: 'Mid-size Economic Areas', code: 'midsize' },
                    { name: 'Small Economic Areas', code: 'small' },
                    { name: 'Manufacturing Intensive Economic Areas', code: 'manufacturing' },
                  ]});

      buttons.push({name: 'Cluster Specialization', code: 'specialization', prefix: 'Economic Areas Specialized in ', items: clusterData });
      buttons.push({name: 'Neighborhood', code: 'neighborhood' , items: [
                      { name: 'Contiguous Economic Areas', code: 'contiguous', default: true },
                      { name: 'Nearby Economic Areas', code: 'nearby' }
                    ]});
      ...
      buttons.push({name: 'Size', code: 'size',
                    legend: "(1-5) Metros by quintile - (6-10) Micros by quintile",
                    items: [
                      { name: 'MSAs by 1st Quintile of metros', code: '1', default: true },
                      { name: 'MSAs by 2nd Quintile of metros', code: '2' },
                      { name: 'MSAs by 3rd Quintile of metros', code: '3' },
                      { name: 'MSAs by 4th Quintile of metros', code: '4' },
                      { name: 'MSAs by 5th Quintile of metros', code: '5' },
                      { name: 'MSAs by 1st Quintile of micros', code: '6' },
                      { name: 'MSAs by 2nd Quintile of micros', code: '7' },
                      { name: 'MSAs by 3rd Quintile of micros', code: '8' },
                      { name: 'MSAs by 4th Quintile of micros', code: '9' },
                      { name: 'MSAs by 5th Quintile of micros', code: '10' },
                    ]})
      buttons.push({name: 'Cluster Specialization', code: 'specialization', prefix: 'MSAs Specialized in ', items: clusterData });
      buttons.push({name: 'Neighborhood', code: 'neighborhood' , items: [
                        { name: 'MSAs in the Same State', code: 'state', default: true },
                        { name: 'MSAs in the Same Economic Area', code: 'economic' }
                  ]});

    } else if (type == 'county') {
      
      if (options.filter == 'default') options.filter = 'state';

      buttons.push({ name: 'Counties in the Same State', code: 'state', items: [] });
      buttons.push({ name: 'Counties in the Same Economic Area', code: 'economic', items: [] });
    }
    drawFilterButtons(buttons);
  }

  function drawFilterButtons(buttons) {
    d3.select('.filter-controls').remove();
    var container = d3.selectAll("#controls").append('div').attr('class', 'filter-controls'),
      filter,
      subfilter,
      subfilters = [],
      prefix = '';
    var filterLabel = options.regionType == 'compare' ? 'Region Type for Comparison' : 'Comparison Sets';
    container.append('label').attr('class', 'control-label').text(filterLabel);
    
    filter = container.append('select').attr({class:'form-control', 'data-toggle':'buttons', name: 'filter'});
    filter.selectAll('option').data(buttons).enter()
      .append('option').attr({ value: function(d){return d.code;}}).text(function(d){return d.name;});
    var activeFilter = filter.select('option[value="'+ options.filter + '"]').attr('selected', 'selected');
    options.activeFilter = activeFilter.text();

    for (var i = 0; i < buttons.length; i++) {
      if (options.filter == buttons[i].code) {
        subfilters = buttons[i].items;
        prefix = buttons[i].prefix || "";
      }
    }

    if (subfilters.length) {
      container.append('label').attr('class', 'control-label').text('Comparison Items');
      subfilter = container.append('select').attr({class:'form-control', 'data-toggle':'buttons', name: 'subfilter'});
      subfilter.attr({name:'subfilter', 'class': 'form-control' })
          .selectAll('option').data(subfilters).enter()
          .append('option').attr({ value: function(d){return d.code;}}).text(function(d){return d.name;});
      if (options.subfilter) {
        activeFilter = subfilter.select('option[value="'+ options.subfilter + '"]').attr('selected', 'selected');
      } else {
        activeFilter = subfilter.selectAll('option');
      }
    }
    options.activeFilter = prefix + activeFilter.text();
    setTimeout(function(){
      $('#comparison-subtitle').text(options.activeFilter);
    }, 0);

    d3.select('select[name=filter]').on('change', function() {
      options.filter = this.value; options.subfilter = undefined;
      filterControls();
      options.subfilter = $('select[name="subfilter"]').first().val();
      go(location());
    });

    d3.select('select[name="subfilter"]').on('change', function() {
      options.subfilter = this.value;
      filterControls();
      go(location());
    });
  }
  ...
  function serviceUrl(suffix) {
    var hash = location().substring(1).split('?')[0];
  ...
  function updateLegend() {
    if (! options.regionTypes[options.regionType]); return;
    var text = 'Comparative Performance for the top '
      + (options.regionType != 'state' ? '100 ' : '')
      + options.regionTypes[options.regionType].legend + ' out of '
      + options.regionTypes[options.regionType].count + '.';
