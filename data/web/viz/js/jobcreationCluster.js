...
(function() {
    ...
    plot = d3Plot()
      ...
      .layer('axisLayer',{
        ...
        elements:[
          {
            type: 'x',
            label: function() { return 'Regions' },
            'vertical-tick-label': true,
            attrs:{
              transform: function(d){
                //console.log(d3.select(this).select('text.axis-label')[0][0] )
        ...
        elements:[
          {
            ...
            append: [
              ...
              },
              {
                ...
                text: function(d){if (d) {return 'Job Creation by ' + d.type + ', ' + d.start + '-' + d.end;}}
              }]
          }
        ]})
      .layer('chartNotation', {
            ...
            append: [
              {
                ...
                text: function(d){if (d) {return 'Net ' + (d.type == 'State' ? 'US' : d.type) + ' Job Creation, ' + d.start + ' to ' + d.end + ': ' + plusFormat(d.value);}}
              },
              /*{
                type: 'h4',
                attrs: {class: 'title-text'},
                text: function(d){if (d.benchmark && options.benchmark) {return "Expected job creation if matching national cluster benchmarks: " + plusFormat(d.benchmark);}}
              }*/
            ]
          }
        ]})
                ...
                append: [
                  {
                    type: 'a',
                    attrs: {
                      href: function(d) { return (options.subcluster ? location(d.id) : '#'); },
                ...
                append: [
                  {
                    type: 'a',
                    attrs: {
                      href: function(d) { return '/cluster/' + d.id; },
                      target: '_parent'
                    },
                    text:  "Go to Cluster Dashboard",
                    append: [
                      {
                        type: 'span',
                ...
                append: [
                  {
                    ...
                    attrs: {
                      href: function(d) {
                        return '/region-cluster/' + ( totalData.clusterParentKey || totalData.clusterKey ) + '/' + d.id;
                      },
                      target: '_parent'
                    },
  ...
  function location(regionType){
    var region = '/' + (regionType || options.regionType),
        ...
        hash = '#' + cluster + subcluster + start + end + region + qops;
    return hash;
  }
  ...
  function getDatasources() {
    return base + '/report/cluster/jobcreation' + window.location.hash.substring(1).split('?')[0];
  }
  ...
  function serviceUrl(suffix) {
    ...
    return window.location.pathname.replace(/viz\/.*/, 'report/jobcreation') +
            hash + '/' + suffix;
  }
