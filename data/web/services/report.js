...
var solr = require('solr-client'),
    ...
    fs = require('fs'),
    fsm = require('mz/fs'),
    tsFormat = d3.timeFormat('%Y%m%d%H%M%S'),
...
function subClusterQ(subcluster) {
    if (subcluster == 'all') {
        ...
        return isNaN(subcluster) ? {subcluster_b: true, key_t: subcluster} : {
            subcluster_b: true,
            sub_code_t: subcluster
        };
    }
}

function analyze(data, region, benchmarkRegion, typeInfo, start, end, keyF, valF, labelF, analysisF, benchmarkF, idNaN) {
    var byRegion = function (d) {
            return d.region_type_t + '/' + (idNaN ? d.region_key_t : d.region_code_t);
        },
        byYear = function (d) {
            return d.year_t;
        },
    ...
    // the indexes are prefixed with a $
    var regionYearArr = byRegionYear.get(region)[`$${start}`];
    Object.keys(regionYearArr).forEach(function (k) {
        var v = regionYearArr[k];
        var startValue = valF(v), endValue = valF(byRegionYear.get(region)[`$${end}`][k]),
            benchmarkStart = valF(byRegionYear.get(benchmarkRegion)[`$${start}`][k]),
            benchmarkEnd = valF(byRegionYear.get(benchmarkRegion)[`$${end}`][k]),
...
function jobCreation(data, region, benchmarkRegion, cluster, start, end, idNaN) {
    var labelF = function (d) {
            return (d.sub_name_t ? d.sub_name_t : d.cluster_name_t);
        },
        keyF = function (d) {
            return d.cluster_code_t + (d.sub_code_t ? '-' + d.sub_code_t : '');
        },
        valF = function (d) {
            return (d ? d.emp_tl : 0)
        },
        analysisF = function (start, end) {
            return end - start;
        },
...
function loadJobCreation(client) {
    return function (req) {
        var type = req.params.type,
            ...
            idQry = isNaN(+code) ? {region_type_t: type, region_key_t: code} : {
                region_type_t: type,
                region_code_t: code
            },
...
function captureJobCreation(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.type + '/' + p.code + '/' + p.start + '/' + p.end + '/' + p.cluster + "",
    ...
    const p = req.params;
    const filename = 'jobcreation_' + [p.type, p.code, p.start, p.end, p.cluster, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/jobcreation`;
    const hash = `#/${p.type}/${p.code}/${p.start}/${p.end}/${p.cluster}`;

    screenshot(req, res, next, path + hash, filename);
}
...
function jobCreationCSV(client) {
    return function (req, res) {
        ...
        return deferQuery(client, query, function (queryResult) {
            var results = jobCreation(queryResult.response.docs, region, benchmarkRegion, req.params.cluster, start, end, isNaN(+code)),
                json2csv;
            ...
            json2csv = new SimpleJson2Csv({
                ...
                transform: function (d) {
                    d.benchmark = benchmarkFormat(d.benchmark);
                    return d;
                }
            });
...
function analyzeCluster(data, benchmarkRegion, start, end, valF, labelF, analysisF, benchmarkF) {
    var byRegion = function (d) {
            return d.region_type_t + '/' + d.region_key_t;
        },
        byYear = function (d) {
            return d.year_t;
        },
    ...
    Object.keys(byYearRegion.get(start)).forEach(function (k) {
        var v = byYearRegion.get(start)[k];
        ...
        var startValue = valF(v), endValue = valF(byYearRegion[`$${end}`][k]),
            value = analysisF(startValue, endValue, benchmarkStart, benchmarkEnd),
            benchmark = benchmarkF(startValue, endValue, benchmarkStart, benchmarkEnd);
        total.value += value;
        total.benchmark += (isNaN(benchmark) ? 0 : benchmark);
    ...
    result = result.sort(function (a, b) {
        return d3.descending(a.absvalue, b.absvalue);
    });
...
function jobCreationCluster(data, benchmarkRegion, start, end) {
    var labelF = function (d) {
            return (d.region_short_name_t ? d.region_short_name_t : d.region_name_t);
        },
        valF = function (d) {
            return (d ? d.emp_tl : 0)
        },
...
function captureEmployment(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.type + '/' + p.code + '/' + p.year + '/' + p.cluster + "",
    ...
    const p = req.params;
    const filename = 'employment_' + [p.type, p.code, p.year, p.cluster, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/employment`;
    const hash = `#/${p.type}/${p.code}/${p.year}/${p.cluster}`;
...
function captureEmploymentCluster(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.cluster + '/' + p.subcluster + '/' + p.year + '/' + p.type + "",
    ...
    const p = req.params;
    const filename = 'employment_cluster_' + [p.cluster, p.subcluster, p.code, p.year, p.type, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/cluster/employment`;
    const hash = `#/${p.cluster}/${p.subcluster}/${p.year}/${p.type}`;
...
function getBenchmarkWageTotal(client, type, year, cb) {
    var qry = q({type_t: 'aggregate'})
            .and({year_t: year})
            .and(q({region_type_t: 'country', region_code_t: '98'})),
        query = client.createQuery().rows(100).q(qry.q());
...
function wages(data, region, benchmarkRegion, year, typeInfo, totals, idNaN) {
    var labelF = function (d) {
            return (d.sub_name_t ? d.sub_name_t : d.cluster_name_t);
        },
        keyF = function (d) {
    ...
    Object.keys(byRegion.get(region)).forEach(function (k) {
        var v = byRegion.get(region)[k];
        var val = valF(v), benchmark = valF(byRegion.get(benchmarkRegion)[k]);
    ...
    total.benchmark = totals.benchmark;
    return {
        totals: total,
        results: result.sort(function (a, b) {
            return d3.descending(a.value, b.value);
    ...
    return function (req) {
        ...
        return getBenchmarkWageTotal(client, req.params.cluster, year).then(function (result) {
            ...
            return getRegionWageTotal(client, type, code, req.params.cluster, year).then(function (result) {
                ...
                return deferQuery(client, query, function (queryResult) {
                    var results = wages(queryResult.response.docs, region, benchmarkRegion, year, req.params.cluster, totals, isNaN(+code));
                    if (cb) {
                        cb(results);
                    }
                    else {
...
function captureWages(req, res, next) {
    ...
    const p = req.params;
    const filename = 'wages_' + [p.type, p.code, p.year, p.cluster, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/wages`;
    const hash = `#/${p.type}/${p.code}/${p.year}/${p.cluster}`;
...
function captureWagesCluster(req, res, next) {
    ...
    const p = req.params;
    const filename = 'wages_' + [p.cluster, p.subcluster, p.year, p.type, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/cluster/wages`;
    const hash = `#/${p.cluster}/${p.subcluster}/${p.year}/${p.type}`;
...
function specialization(data, region, benchmarkRegion, typeInfo, start, end, idNaN) {
    var labelF = function (d) {
        ...
        totalBenchmarkEmployment = {
            start: d3.sum(byRegionYear.get(benchmarkRegion).get(start).values(), valF),
...
function captureSpecialization(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.type + '/' + p.code + '/' + p.start + '/' + p.end + '/' + p.cluster + '/' + p.zoom + "",
    ...
    const p = req.params;
    const filename = 'specialization' + [p.type, p.code, p.start, p.end, p.zoom, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/specialization`;
    const hash = `#/${p.type}/${p.code}/${p.start}/${p.end}/${p.cluster}/${p.zoom}`;
            ...
            loader = loadSpecialization(client, function (results) {
                ...
                json2csv = new SimpleJson2Csv({
                    fields: [
                        ...
                        {
                            name: "change",
                            header: results.totals.region + " Change in Employment share," + p.start + '-' + p.end
                        },
                        {name: "start", header: p.start + " Employment"},
                        {name: "end", header: p.end + " Employment"},
                        {
                            name: "gainloss",
                            header: results.totals.region + " Change in Employment," + p.start + '-' + p.end
                        },
                    ...
                    data: results.results,
                    transform: function (d) {
                        d.value = percentFormat(d.value);
                        d.benchmark = percentFormat(d.change);
                        return d;
                    }
                });
                // Add credit to end of file.
...
function specializationCluster(data, benchmarkRegion, start, end, idNaN) {
    var labelF = function (d) {
        ...
        byRegion = function (d) {
            return d.region_type_t + '/' + d.region_code_t;
        },
        byYear = function (d) {
            return d.year_t;
        },
...
function captureSpecializationCluster(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.cluster + '/' + p.subcluster + '/' + p.start + '/' + p.end + '/' + p.type + '/' + p.zoom + "",
    ...
    const p = req.params;
    const filename = 'specialization' + [p.cluster, p.subcluster, p.start, p.end, p.type, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/cluster/specialization`;
    const hash = `#/${p.cluster}/${p.subcluster}/${p.start}/${p.end}/${p.type}/${p.zoom}`;
...
function scorecard(data, region, benchmarkRegion, indicator, start, end, filters, regionName) {
    var resultLimit = 60,
        labelF = function (v) {
            return (v.region_short_name_t ? v.region_short_name_t : v.region_name_t);
        },
    ...
    Object.keys(byRegionYear).forEach(function (k) {
        var v = byRegionYear[k];
        if (k === benchmarkRegion) return;
        var s = v[`$${start}`], e = v[`$${end}`];

         var obj = {
            id: k.charAt(0) === '$'? k.substr(1) : k,
            emp_tl: emplF(e) || emplF(s),
            manufacturing: manuF(e),
            strong_clusters: strongF(e) || strongF(s),
            label: labelF(e),
            key: e.region_key_t,
            value: valF(e),
            change: simpleChange(valF(s), valF(e)),
            cagr: cagr(valF(s), valF(e), end - start),
            start: valF(s),
            end: valF(e),
            regionType: e.region_type_t
        };
    ...
    if (filter == 'specialization') {
        result = result.sort(function (a, b) {
            return d3.descending(a.emp_tl, b.emp_tl);
        });
        ...
        if (subfilter == 'manufacturing') {
            result = result.sort(function (a, b) {
                return d3.descending(a.manufacturing, b.manufacturing);
            });
            top = 0;
            bottom = 50;
        } else {
        ...
        if (type == 'compare') {
            return getComparison(client, code)
                .then(function (comparison) {
                    region = comparison.region_data.map(function (r) {
                        return r.type + '/' + r.code
                    });
                    var qryCompare = comparison.region_data.map(function (r) {
                            return '(region_type_t:' + r.type + ' AND region_code_t:' + r.code + ')'
                        }).join(' OR '),
            ...
            return getContiguousCounties(client, type, code, filter)
                .then(function (regionIds) {
                    qry = qry.and(qryBenchmark.or(q('region_type_t:' + type + ' AND region_code_t:('
                        + regionIds.map(function (c) {
                            return c.replace('region/' + type + '/', '');
                        }).join(' OR ') + ')')));
                    query = client.createQuery().rows(10000).q(qry.q()).fl(fields);
            ...
            return getNearbyRegions(client, type, code, filter.split('-')[1])
                .then(function (regionIds) {
                    qry = qry.and(qryBenchmark
                        .or(q('region_type_t:economic AND region_code_t:(' + regionIds.map(function (c) {
                            return c.replace('region/economic/', '');
                        }).join(' OR ') + ')'))
...
function getTradedClusters(client) {
    ...
    return deferQuery(client, query, extractDocs(function (d) {
        return {code: d.cluster_code_t, key: d.key_t, name: d.name_t};
    }));
}
    ...
    if (filter == 'state') {
        return stateForOther(client, type, code)
            .then(function (stateCode) {
                return regionsForState(client, stateCode, type);
            });
...
function getNearbyRegions(client, type, code, filter) {
    if (filter == 'nearby') {
        ...
        return deferQuery(client, query, function (result) {
            return result.response.docs[0].state_codes_txt;
        })
            .then(function (stateCodes) {
                if (type == 'custom') type = 'economic';
                var query = client.createQuery().rows(10000).q({
                    type_t: 'region',
                    region_type_t: type,
                    state_codes_txt: '(' + stateCodes.join(' OR ') + ')'
                })
                    .fl(['region_code_t']);
                return deferQuery(client, query, extractDocs(function (d) {
                    return d.region_code_t
            ...
            loader = loadScorecard(client, function (results) {
                ...
                json2csv = new SimpleJson2Csv({
                    fields: [
                        ...
                        {
                            name: "cagr",
                            header: (p.indicator == "unemployment" ? "Change in Unemployment Rate" : "Real Growth in " + indicator.label)
                        }
                    ],
                    data: results.results,
                    transform: function (d) {
...
function captureScorecard(req, res, next) {
    ...
    const p = req.params;
    const filename = 'scorecard_' + [p.indicator, p.type, p.code, p.start, p.end, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/scorecard`;
    const hash = `#/${p.type}/${p.code}/${p.start}/${p.end}/${p.indicator}/${p.filter}/${p.zoom}`;
...
function sparkline(data, region, indicator, rank_key, change_rank_key, rank_top) {
    var totals = {
        ...
        yearRange = d3.extent(filteredData.filter(function (d) {
            return byRegion(d) == region;
        }), byYear),
...
function captureSparkline(req, res, next) {
    ...
    const p = req.params;
    const filename = 'sparkline_' + [p.type, p.code, p.indicator, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/spark`;
    const hash = `#/${p.type}/${p.code}/${p.indicator}`;
    const option = "sparkline";

    screenshot(req, res, next, path + hash, filename, option);
}
...
function capturePerformance(req, res, next) {
    ...
    const p = req.params;
    const filename = 'performance_' + [p.type, p.code, tsFormat(new Date())].join('_') + '.' + p.ext;
    const path = `/region/${p.type}/${p.code}/performance`;
    const option = "noheader";

    screenshot(req, res, next, path, filename, option);
}
        ...
        function _processResults(queryResult) {
            ...
            if (key == 'venture_capital_per_gdp_tf') {
                ...
                var regions = Object.keys(byRegion).map(function (k) {
                    var years = byRegion[k].sort(function (a, b) {
                        return d3.ascending(a.year_t, b.year_t);
                    });
                    if (years.length < 2) {
                        return;
                    }
                    ...
                    .sort(function (a, b) {
                        return d3.descending(a.cagr, b.cagr);
                    })
                    .map(function (d, i) {
                        d.change_rank = i + 1;
                        return d;
                ...
                var regionTotals = regions.filter(function (d) {
                    return d.code == code
                }).shift();

                data = data.filter(function (d) {
                    return +d.region_code_t == code
                })
                    .map(function (d) {
...
function regionsForState(client, state_code, type) {
...
function aggregatesForIds(client, type, code, ids, start, end, key) {
    ...
    aggsBase = ids.map(function (d) {
        return d.replace('region', 'aggregate');
    });
    aggsStart = aggsBase.map(function (d) {
        return d + '/' + start;
    });
    aggsEnd = aggsBase.map(function (d) {
        return d + '/' + end;
    });
...
function subregions(data, indicator, start, end, regionName, benchmarkRegion, region) {
    var labelF = function (v) {
        ...
        valF = function (d) {
            return (d ? d[valKey] : undefined);
        },
        wageF = function (d) {
            return d.private_wage_tf;
        },
        byRegion = function (d) {
            return d.region_type_t + '/' + d.region_code_t;
        },
    ...
    Object.keys(byRegionYear).reverse().forEach(function (k) {
        var v = byRegionYear[k];
        ...
        if (type === 'state') {
            return getRegionName(client, type, code)
                .then(function (regionNameResult) {
                    ...
                    return regionsForState(client, regionNameResult.region_code_t, subtype);
                })
                .then(function (regionIds) {
...
function captureSubregions(req, res, next) {
    const p = req.params;
    const filename = 'subregions_' + [p.type, p.code, p.start, p.end, p.subtype, p.indicator, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/subregions`;
    const hash = `#/${p.type}/${p.code}/${p.start}/${p.end}/${p.subtype}/${p.indicator}/${p.zoom ? p.zoom : '1'}`;
...
function timeline(data, dataType, typeInfo, indicator) {
    var vObj = {
        ...
        valF = function (d) {
            return (d ? d[indicator.range_source || indicator.key] : 0)
        },
        byYear = function (d) {
            return +d.year_t;
        },
    ...
    Object.keys(byTypeYear).forEach(function (cid) {
        var set = byTypeYear[cid];
        var cluster;

        years.forEach(function (year, index) {

            var item = _.filter(set, function (d) {
                return +d.year_t == +year
            });
        ...
        cluster.values.sort(function (a, b) {
...
function captureTimeline(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.type + '/' + p.code + '/' + p.cluster + '/' + p.indicator + "",
    ...
    const p = req.params;
    const filename = 'timeline_' + [p.type, p.code, p.cluster, p.indicator, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/timeline`;
    const hash = `#/${p.type}/${p.code}/${p.cluster}/${p.indicator}`;
...
function captureTimelineCluster(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.cluster + '/' + p.subcluster + '/' + p.type + '/' + p.indicator + "",
    ...
    const p = req.params;
    const filename = 'timeline_' + [p.cluster, p.subcluster, p.type, p.indicator, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/cluster/timeline`;
    const hash = `#/${p.cluster}/${p.subcluster}/${p.type}/${p.indicator}`;
...
function loadIndustries(client, cluster, subcluster) {
    ...
    return deferQuery(client, query, function (queryResults) {
        ...
        return {
            ...
            query: {naics_t: '(' + Object.keys(industries).join(' OR ') + ')'}
        };
    });
}
    ...
    if (type !== 'state' && type !== 'county' && type != 'country') {
        return countiesForOther(client, type, code)
            .then(function (counties) {
                var countiesQ = '(' + counties.map(function (c) {
                    return c.slice(-5);
                }).join(' OR ') + ')';
            ...
            .spread(function (industries, regions) {
                ...
                return deferQuery(client, query, function (queryResult) {
                    ...
                    queryResult.response.docs.forEach(function (doc) {
                        ...
                        industriesTable[key][doc.year_t] = {
                            emp: doc.emp_tl,
                            emp_sup: doc.empl_suppressed_tl,
                            est: doc.est_tl,
                            ap: doc.ap_tl
                        };
                    ...
                    return {
                        ...
                        clusterName: industries.clusterName,
...
function processMapData(data, cluster, subcluster, startYear, endYear, indicator, rangeIndicator) {
    ...
    a = endByRegionId.values().sort(function (a, b) {
        return d3.ascending(a.region_code_t, b.region_code_t);
    });
...
function captureMap(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.maptype + '/' + p.start + '/' + p.end + '/' + p.regiontype + '/' + p.regioncode + '/' + p.cluster + '/' + p.subcluster + '/' + p.indicator + '/' + p.zoom + "",
    ...
    const p = req.params;
    const filename = 'map_' + [p.regiontype, p.regioncode, p.cluster, p.subcluster, p.start, p.end, p.indicator, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/map`;
    const hash = `#/${p.maptype}/${p.start}/${p.end}/${p.regiontype}/${p.regioncode}/${p.cluster}/${p.subcluster}/${p.indicator}/${p.zoom}`;
...
function captureInnovation(req, res, next) {
    ...
    const p = req.params;
    const filename = 'innovation' + [p.type, p.code, p.start, p.end, p.cluster, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/region/innovation`;
    const hash = `#/${p.type}/${p.code}/${p.start}/${p.end}/${p.cluster}/${p.zoom}`;
...
function innovationCluster(data, start, end, idNaN) {
    ...
    var regionYearArr = byYearRegion[`$${start}`];
    Object.keys(regionYearArr).forEach(function (k) {
        var v = regionYearArr[k];
        var startValue = valF(v), endValue = valF(byYearRegion[`$${end}`][k]),
            gainloss = endValue - startValue;
...
function captureInnovationCluster(req, res, next) {
    // var p = req.params,
    //     hash = '#/' + p.cluster + '/' + p.start + '/' + p.end + '/' + p.type + '/' + p.zoom + "",
    ...
    const p = req.params;
    const filename = 'innovation' + [p.cluster, p.start, p.end, p.type, tsFormat(new Date())].join('_') + '.png';
    const path = `${base}/report/cluster/innovation`;
    const hash = `#/${p.cluster}/${p.start}/${p.end}/${p.type}/${p.zoom}`;
...
function compareRegions(comparison, data, key, start, end) {
    var result = [],
        byRegion = function (d) {
            return 'region/' + d.region_type_t + '/' + d.region_code_t;
        },
        byCluster = function (d) {
            return d.key_t;
        },
        byYear = function (d) {
            return d.year_t;
        ...
        // Load Aggregates
            .then(function (comparison) {
                ...
                var key = indicator.range_source || indicator.key,
                    qry = q(q(comparison.region_data.map(function (d) {
                        return '(region_type_t:' + d.type + ' AND region_code_t:' + d.code + ')';
                    }).join(' OR ')))
        ...
        if (p.type && p.code) {
            var qry2 = q({
                ...
                p2 = deferQuery(client, query2, function (queryResult) {
                    var data = queryResult.response.docs.filter(function (d) {
                            return d.lq_tf && d.emp_tl;
                        }),
                        byCluster = function (d) {
                            return d.cluster_code_t;
                        },
                        ...
                        filteredData = data
                            .filter(function (d) {
                                return d.year_t == yearMax
                            })
                            .sort(function (a, b) {
                                return d3.descending(a.lq_tf, b.lq_tf);
                            });
        ...
        var p = req.params,
            loader = loadComparison(client, function (comparison) {
                ...
                comparison.region_data.forEach(function (r) {
                    ...
                    fields.push({
                        name: r.id + "_percent",
                        header: r.name + " % of total region " + comparison.indicator.label
                    });
                });
                // Add credit to end of file.
...
function captureUrl(req, res, next) {
    ...
    const p = req.params;
    const filename = p.name + '_' + tsFormat(new Date()) + '.png';
    const url = decodeURIComponent(p.url);
    
    screenshot(req, res, next, url, filename);
}

function screenshot(req, res, next, path, filename, option) {
    let url = 'http://' + baseUrl + path;    
    let viewportSize = {width: 1280, height: 1024};
    ...
    (async () => {
      try {
        ...
        await page.evaluate(function () {
            jQuery('.map-menu, .nav-controls, .year-control, .cluster-controls, .download-controls, .layer-downloadControls').hide();
        });
        ...
        await page.screenshot({path: tmpFilepath, fullPage: true});
        await sendFile(tmpFilepath, filename, res, next);
        await browser.close();
      } catch (e) {
        console.error(e);
      }
